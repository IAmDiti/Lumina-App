      ...
      <section className="mx-auto w-full max-w-5xl px-4 pb-20 sm:px-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {STEPS.map((step, i) => (
            <div
              key={step.title}
              className="group relative rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6 transition-all duration-200 hover:-translate-y-1 hover:border-indigo-400/30 hover:bg-zinc-900/50 hover:shadow-xl hover:shadow-indigo-950/40"
            >
              <span className="font-mono text-xs text-indigo-300/70">{String(i + 1).padStart(2, "0")}</span>
              <h2 className="mt-2 font-serif text-xl text-white transition-colors">{step.title}</h2>
